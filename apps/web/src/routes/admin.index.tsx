...
function AdminDashboard() {
  ...
  return (
    <div className="container mx-auto max-w-6xl px-4 py-8 space-y-6">
      <Badge className="mb-2">Control room</Badge>
      <h1 className="text-4xl font-black tracking-tight md:text-5xl">Admin Dashboard</h1>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>{reports.data?.users ?? "—"}</CardContent>
        </Card>
        <Card>
          <CardHeader>
        ...
        <Card>
          ...
          <CardContent>{reports.data?.openComplaints ?? "—"}</CardContent>
        </Card>
      </div>
        ...
        <CardContent className="space-y-2">
          ...
          {pendingLandlords.data?.map((p) => (
            <div key={p.id} className="flex items-center justify-between border-b pb-2">
              <span className="font-bold">{p.businessName ?? p.id}</span>
